...
The event has a property `media` that holds the `\Develoopin\MediaLibrary\Models\Media`-object of which the file has been stored.
...
- `media`: the `\Develoopin\MediaLibrary\Models\Media`-object of which a conversion will be started
- `conversion`: the conversion (an instance of `\Develoopin\MediaLibrary\Conversion\Conversion`) that will start
...
- `media`: the `\Develoopin\MediaLibrary\Models\Media`-object of which a conversion has been completed
- `conversion`: the conversion (an instance of `\Develoopin\MediaLibrary\Conversion\Conversion`) that has just been completed
...
- `model`:  the object that conforms to `\Develoopin\MediaLibrary\HasMedia\Interfaces\HasMedia` of which a collection has just been cleared.
...
```php
protected $listen = [
    'Develoopin\MediaLibrary\MediaCollections\Events\MediaHasBeenAdded' => [
